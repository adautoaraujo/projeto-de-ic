...
def dimensoes_tanque(vol_m3,altura_adotada):
    ...
    print("As dimensões do tanque séptico são: altura= %.2f m, comprimento= %.2f m e largura= %.2f m"%(altura_adotada,comprimento,largura))
...
def filtro_anaerobio(n_contri,contri_despejos,p_retencao):
    ...
    print("As dimensões do filtro anaeróbio são: altura= %.2f m, comprimento= %.2f m e largura= %.2f m"%(h_filtro,c_filtro,l_filtro))
...
def modulo_principal():
    ...
    while(inter_limp<1)or(inter_limp>5):
        ...
        inter_limp= int(input("Insira o intervalo de limpeza do tanque séptico em anos:"))
    t_amb= int(input("Insira a temperatura ambiente do mês mais frio em ºC:"))
...
#dicionario padrão carrega em suas listas os valores de contribuição de despejos e contribuição de lodo, nessa ordem
    dic_padrao={"alto":[160,1],"medio":[130,1],"baixo":[100,1]}
    contri_despejos=dic_padrao[padrao][0]
    contri_lodo=dic_padrao[padrao][1]
#dicionario intervalo carrega as informações de tempo de acumulação de acordo com a temperatura
    ...
    print("O volume útil do tanque séptico prismático é de","%.2f"%tanque_septico(n_contri,contri_despejos,contri_lodo,p_retencao,t_acumulacao),"L ou",end=" ")
    print("%.2f"%vol_m3,"m³")
